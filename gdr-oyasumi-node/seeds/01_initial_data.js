...
exports.seed = async function(knex) {
  ...
  // *Questo blocca pulisce TUTTO, inclusi i nuovi utenti.*
  ...
  // 2. INSERISCE GLI UTENTI con ID fissi (Necessario per FK nelle altre tabelle)
  ...
  // =================================================================
  // --- CORREZIONE CRITICA DELLA SEQUENZA ---
  // Aggiunge i comandi per resettare i contatori (sequence) di PostgreSQL
  // Questo garantisce che il prossimo ID generato parta dal MAX(ID) + 1.
  console.log("Aggiornamento delle sequenze del database...");

  // Tabella Utenti (Usa id_utente come PK) - Questo dice al database: "il prossimo utente è ID 3"
  await knex.raw("SELECT setval('utenti_id_utente_seq', (SELECT MAX(id_utente) FROM utenti));");
  
  // Resetta anche gli altri contatori per sicurezza
  await knex.raw("SELECT setval('locations_id_seq', (SELECT MAX(id) FROM locations));");
  await knex.raw("SELECT setval('forum_sezioni_id_seq', (SELECT MAX(id) FROM forum_sezioni));");
  await knex.raw("SELECT setval('forum_bacheche_id_seq', (SELECT MAX(id) FROM forum_bacheche));");
  await knex.raw("SELECT setval('forum_topics_id_seq', (SELECT MAX(id) FROM forum_topics));");
  await knex.raw("SELECT setval('forum_posts_id_seq', (SELECT MAX(id) FROM forum_posts));");
  await knex.raw("SELECT setval('quests_id_seq', (SELECT MAX(id) FROM quests));");

  console.log("Sequenze del database aggiornate per ripartire correttamente.");
};