...
const getStyles = (isMobile) => {
    // 1. STILI MOBILE (WhatsApp Style)
    if (isMobile) {
        return {
            window: { 
                position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', 
                backgroundColor: '#0b0b0b', zIndex: 2000, display: 'flex', flexDirection: 'column',
                fontFamily: "'Inter', sans-serif"
            },
            header: { 
                padding: '15px', backgroundColor: '#1f1f22', borderBottom: '1px solid #333', 
                display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 
            },
            headerTitle: { fontFamily: "'Cinzel', serif", color: '#c9a84a', fontWeight: '700', fontSize: '16px' },
            closeBtn: { background: 'none', border: 'none', color: '#888', fontSize: '20px' },
            
            mainContent: { flexGrow: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' },
            messages: { 
                flexGrow: 1, padding: '15px', overflowY: 'auto', 
                backgroundImage: "url('/backgrounds/chat-pattern.png')", backgroundBlendMode: 'overlay', backgroundColor: '#0b0b0b' 
            },
            
            inputArea: { 
                padding: '10px', backgroundColor: '#1f1f22', borderTop: '1px solid #333', flexShrink: 0 
            },
            form: { display: 'flex', gap: '10px' },
            luogoInput: { 
                width: '80px', padding: '10px', borderRadius: '8px', border: 'none', background: '#2a2a2e', color: '#c9a84a', fontSize: '11px', textAlign: 'center' 
            },
            messageInput: { 
                flexGrow: 1, padding: '10px', borderRadius: '20px', border: 'none', background: '#2a2a2e', color: 'white', fontSize: '14px', resize: 'none', height: '40px' 
            },
            button: { 
                width: '40px', height: '40px', borderRadius: '50%', border: 'none', background: '#c9a84a', color: '#000', fontWeight: 'bold', fontSize: '18px' 
            }
        };
    }

    // 2. STILI DESKTOP (Dark Arcane - Larghezza Main)
    return {
        window: { 
            position: 'absolute', 
            // ABBASSATA: da 100px a 130px
            top: '155px', 
            left: '49.2%', 
            transform: 'translateX(-50%)',
            
            // LARGHEZZA DINAMICA: 100% meno lo spazio delle due sidebar (circa 290px * 2)
        ...
        leftColumn: { 
            width: '280px', flexShrink: 0, padding: '20px', 
        ...
        inputArea: { 
            ...
            backgroundColor: 'rgba(15, 15, 20, 0.95)', flexShrink: 0 
        },
        ...
        modalHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(201, 168, 74, 0.3)', paddingBottom: '10px' },
        ...
        inventoryItem: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '2px' },
        ...
        stealBtn: { padding: '5px 12px', background: 'rgba(255, 42, 42, 0.1)', border: '1px solid #ff2a2a', borderRadius: '2px', color: '#ff4444', cursor: 'pointer', fontSize: '10px', fontFamily: "'Cinzel', serif", fontWeight: 'bold', transition: 'all 0.2s' }
    };
};

function ChatWindow({ chat, onClose, user, isMobile, onMessageSent }) {
  ...
  useEffect(() => {
    const fetchLocationDetails = async () => {
        try {
            if (isHouseChat) {
                ...
                setLocationDetails({
                    ...
                    image_url: response.data.image_url || '/placeholder.jpg'
                });
        ...
        } catch (error) { console.error("Errore recupero dettagli:", error); }
    };
    ...
    const fetchHistory = async () => { 
        ...
        } catch (error) { console.error("Errore cronologia", error); } 
    };
    ...
    const handleNewMessage = (data) => { 
        if (data.tipo === 'globale' || data.chatId === chat.id) setMessages(prev => [...prev, data]); 
    };
  ...
  const handleOpenHouseInventory = async () => {
      ...
      } catch (e) { alert(e.response?.data?.message || "Impossibile aprire l'armadio."); }
  };

  const handleStealItem = async (itemId) => {
      if (!confirm("Sei sicuro di voler prendere questo oggetto?")) return;
      try {
          const res = await api.post('/housing/steal', { invItemId: itemId, targetChatId: chat.id });
          alert(res.data.message);
          handleOpenHouseInventory(); 
      } catch (e) { alert(e.response?.data?.message || "Errore durante l'azione."); }
  };
  ...
  const handleSendMessage = (messageType) => {
    ...
    socket.emit('send_message', { chatId: chat.id, testo: currentMessage, tipo: finalMessageType, luogo: currentLuogo, quest_id: activeQuest?.questId });
    setCurrentMessage("");
    if (onMessageSent) onMessageSent();
  };

  const handleFormSubmit = (e) => { e.preventDefault(); handleSendMessage('azione'); };
  const handleSaveNotes = async () => { try { await api.put(`/chats/${chat.id}/notes`, { master_notes: masterNotes }); alert('Note salvate!'); } catch (error) { console.error("Errore note", error); } };
  const handleDiceRoll = (diceType) => { socket.emit('roll_dice', { chatId: chat.id, diceType: diceType }); setShowDiceMenu(false); };
  const handleGlobalClick = () => { handleSendMessage('globale'); setGlobalGifKey(p => p + 1); };
  ...
  // --- RENDER MOBILE (SEMPLIFICATO) ---
  if (isMobile) {
      return (
        <div style={styles.window}>
            <div style={styles.header}>
                <div style={styles.headerTitle}>💬 {locationDetails?.name || chat.name}</div>
            ...
            <div style={styles.mainContent}>
                ...
                <div style={styles.inputArea}>
                    <form onSubmit={handleFormSubmit} style={styles.form}>
                        <input value={currentLuogo} onChange={e=>setCurrentLuogo(e.target.value)} placeholder="Luogo" style={styles.luogoInput} />
                        <textarea value={currentMessage} onChange={e=>setCurrentMessage(e.target.value)} placeholder="Messaggio..." style={styles.messageInput} />
    ...
    <div style={styles.window}>
                          ...
                          houseInventory.items.map(item => (
                              <div key={item.inv_id} style={styles.inventoryItem}>
                                  <div style={styles.itemInfo}>
                                      ...
                                      <div><div style={styles.itemName}>{item.nome}</div><div style={styles.itemQty}>x{item.quantita}</div></div>
                                  </div>
                                  <button style={styles.stealBtn} onClick={() => handleStealItem(item.inv_id)}>RUBA</button>
                              </div>
                          ))
                      ) : <div style={{textAlign:'center', color:'#666', padding:'20px'}}>Vuoto.</div>}
                  </div>
              </div>
          </div>
      )}
      ...
      <div style={styles.header}>
        <div style={styles.headerTitle}>
             <span style={{color:'#60519b'}}>💬</span> {locationDetails?.name || chat.name}
             {activeQuest && <span style={{color:'#ff4d4d', fontSize:'10px', border:'1px solid #ff4d4d', padding:'2px 5px', borderRadius:'3px'}}>QUEST</span>}
        </div>
        <button style={styles.closeBtn} onClick={() => onClose(chat.id)}>✕</button>
      </div>
      ...
      <div style={styles.mainContent}>
        <div style={styles.leftColumn}>
          ...
          <div style={styles.descriptionBox}>{locationDetails?.description || '...'}</div>
          
          <button style={styles.toolButton} onClick={() => alert("Skill in arrivo!")}><span>✦ SKILL & ITEMS</span></button>
          {isHouseChat && <button style={styles.toolButton} onClick={handleOpenHouseInventory}><span>📦 ARMADIO</span></button>}

          {['MASTER', 'ADMIN'].includes(user.permesso) && chat.type === 'MAP' && (
             <div style={{marginBottom:'15px', display:'flex', flexDirection:'column', gap:'5px'}}>
                <textarea style={{...styles.messageInput, height:'60px', fontSize:'11px'}} value={masterNotes} onChange={(e) => setMasterNotes(e.target.value)} placeholder="Note Master..." />
                <button onClick={handleSaveNotes} style={{...styles.button, height:'25px', fontSize:'10px', padding:'0'}}>SALVA</button>
             </div>
          )}
          
          <div style={styles.presentiBox}>
              <div style={styles.presentiTitle}>PRESENTI ({usersInRoom.length})</div>
              <ul style={styles.presentiList}>{usersInRoom.map(u => <li key={u.id}>• {u.nome_pg}</li>)}</ul>
          </div>
        </div>
         ...
         <div style={styles.rightColumn}>
          ...
          <div style={styles.inputArea}>
            <form onSubmit={handleFormSubmit} style={styles.form}>
                <input type="text" placeholder="Luogo..." style={styles.luogoInput} value={currentLuogo} onChange={(e) => setCurrentLuogo(e.target.value)} />
                <textarea placeholder="Azione..." style={styles.messageInput} value={currentMessage} onChange={(e) => setCurrentMessage(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleFormSubmit(e); } }} rows="1" />
            ...
            <div style={styles.bottomBar}>
              <div style={styles.toolsLeft}>
                  <div style={styles.diceDropdownContainer}>
                    <button style={styles.iconButton} onClick={() => setShowDiceMenu(!showDiceMenu)}>🎲</button>
                    {showDiceMenu && (
                      <div style={styles.diceDropdownMenu}>
                        {[6,10,20,100].map(d => <button key={d} style={styles.diceButton} onClick={() => handleDiceRoll(d)}>d{d}</button>)}
                      </div>
                    )}
                  </div>
                  {['MASTER', 'ADMIN'].includes(user.permesso) && <button style={styles.iconButton} onClick={() => setIsShinigamiPanelOpen(true)}>💀</button>}
                  {user.permesso === 'ADMIN' && <button style={styles.iconButton} onClick={handleGlobalClick}>🌍</button>}
              </div>
