...
// --- STILI CSS-IN-JS (PULITI E UNIFICATI) ---
const styles = {
  ...
  // --- FIX: Rimosse chiavi duplicate di input, textarea, btnSave, resetBtn, gridSlot, inventoryGrid ---
...
function SchedaPersonaggio({ user, datiScheda, onClose, targetUser, onOpenChat }) {
    ...
    }, [activeSection, fullData, isMyProfile]);
    ...
    const handleEnterHouse = () => {
        ...
        onOpenChat({ type: 'CHAT', id: chatId, name: `Abitazione: ${houseName}` });
    };
    ...
    const calcolaDerivate = (s) => { 
        const lancioBase = (s.forza * 0.7) + (s.destrezza * 0.3); 
        return { 
            stat_body: 1 * ((s.forza * 0.5) + (s.costituzione * 2)), 
            stat_kotodama: 3 * ((s.mente * 0.6) + (s.empatia * 0.4)), 
            reflexes: 1 * ((s.mente * 0.4) + (s.destrezza * 0.6)), 
            velocita: 1 * ((s.forza * 0.4) + (s.destrezza * 0.6)), 
            movimento: 0.5 * ((s.forza * 0.5) + (s.costituzione * 0.6)), 
            salto: 0.4 * ((s.forza * 0.5) + (s.costituzione * 0.6)), 
            lancio: 0.45 * lancioBase, 
            lancio_base_calc: lancioBase, 
            peso_trasportabile: 2 * ((s.forza * 0.6) + (s.costituzione * 0.4)), 
            ingaggio: 0.1 * ((s.forza * 0.4) + (s.destrezza * 0.6)), 
            percezione_sensi: 1 * ((s.mente * 0.9) + (s.costituzione * 0.4)), 
            percezione_spirituale: 1 * ((s.mente * 0.9) + (s.empatia * 0.4)), 
            danno_cac: 1 * ((s.forza * 0.3) + (s.destrezza * 0.2)), 
            danno_cad: 1 * ((s.forza * 0.2) + (s.destrezza * 0.3)) 
        }; 
    };

    const derivate = calcolaDerivate(stats);
    const radarData = [ 
        { label: 'RIFLESSI', value: derivate.reflexes, max: STAT_MAX_VALUES.reflexes }, 
        { label: 'VELOCITÀ', value: derivate.velocita, max: STAT_MAX_VALUES.velocita }, 
        { label: 'MOVIM.', value: derivate.movimento, max: STAT_MAX_VALUES.movimento }, 
        { label: 'INGAGGIO', value: derivate.ingaggio, max: STAT_MAX_VALUES.ingaggio }, 
        { label: 'SPIRITO', value: derivate.percezione_spirituale, max: STAT_MAX_VALUES.percezione_spirituale }, 
        { label: 'FISICO', value: derivate.percezione_sensi, max: STAT_MAX_VALUES.percezione_fisica } 
    ];
            ...
            case 'CASA':
                content = (
                    <div style={{padding:'20px', color:'#e0e0e0', display:'flex', flexDirection:'column', gap:'20px'}}>
                        {fullData.housing_id && (
                            <div style={{textAlign:'center', borderBottom:`1px solid ${THEME.colors.panelBorder}`, paddingBottom:'20px'}}>
                                ...
                                <h2 style={{fontFamily:THEME.fonts.title, color:THEME.colors.primary, margin:'0'}}>{houseInfo?.name || fullData.house_name}</h2>
                                <p style={{fontSize:'12px', color:'#888'}}>Residenza Privata</p>
                                <p style={{fontSize:'13px', fontStyle:'italic', color:'#b3b3c0', margin:'15px 0'}}>{fullData.house_custom_desc || "Un luogo sicuro dove riposare."}</p>
                                {(isMyProfile || isStaff) && (
                                    <button style={isStaff && !isMyProfile ? styles.btnAdminHouse : styles.btnHouse} onClick={handleEnterHouse}>
                                ...
                                {isEditMode && isMyProfile && (
                                    <div style={{marginTop:'15px', background:'rgba(0,0,0,0.3)', padding:'10px', borderRadius:'4px'}}>
                                        <div style={styles.formGroup}><label style={styles.label}>FOTO CASA (URL)</label><input style={styles.input} name="image" value={editedHouse.image} onChange={handleEditHouseChange} /></div>
                                        <div style={styles.formGroup}><label style={styles.label}>DESCRIZIONE</label><textarea style={styles.textarea} name="desc" value={editedHouse.desc} onChange={handleEditHouseChange} /></div>
                                ...
                                {isMyProfile && houseGuests && (
                                    <div style={{marginTop:'20px', textAlign:'left', background:'rgba(0,0,0,0.2)', padding:'10px', borderRadius:'4px'}}>
                                        ...
                                        <div style={{display:'flex', gap:'5px', marginBottom:'10px'}}>
                                            <input style={{...styles.input, marginBottom:0, padding:'5px'}} placeholder="Nome" value={inviteName} onChange={(e) => setInviteName(e.target.value)} />
                        ...
                        {isMyProfile && (
                            <div style={{textAlign:'left'}}>
                                ...
                                <div style={styles.inventoryGrid}>
                                    {guestKeys.map((k, i) => (
                                        <div key={i} style={{...styles.gridSlot, width:'100%', height:'40px', justifyContent:'space-between', padding:'0 10px', cursor:'pointer'}} onClick={() => handleEnterGuestHouse(k)}>
    ...
    return (
        <div style={{ ...styles.dragWrapper, left: `${position.x}px`, top: `${position.y}px` }}>
            ...
            {showBanModal && (<div style={styles.modalOverlay}><div style={styles.modalBox}><h4>BAN UTENTE</h4><select style={styles.input} value={banType} onChange={(e) => setBanType(e.target.value)}><option value="FULL">BAN TOTALE</option><option value="SHADOW">SHADOW BAN</option></select><input type="number" style={styles.input} value={banDays} onChange={(e) => setBanDays(e.target.value)} min="1" /><textarea style={{...styles.textarea, minHeight: '100px'}} value={banReason} onChange={(e) => setBanReason(e.target.value)} placeholder="Motivazione..." /><div style={{display:'flex', gap:'10px'}}><button style={{...styles.btnSave, background: THEME.colors.danger, color:'#fff'}} onClick={handleBanUser}>CONFERMA</button><button style={{...styles.btnSave, background: 'transparent', border:'1px solid #666', color:'#ccc'}} onClick={() => setShowBanModal(false)}>ANNULLA</button></div></div></div>)}

            <div style={styles.windowFrame}>
                <div style={styles.header}>
                    <div style={styles.dragHandle} onMouseDown={onMouseDown}>{isMyProfile && <button style={{...styles.editBtnHeader, opacity: isEditMode ? 1 : 0.6}} onClick={() => { setIsEditMode(!isEditMode); if (!activeSection) setActiveSection('BACKGROUND'); }}>✎</button>} SCHEDA: {fullData.nome_pg} {fullData.cognome}</div>
                ...
                <div style={{...styles.contentArea, padding: 0}}>
                    <div style={styles.headerSection}>
                        ...
                        <div style={styles.infoBlock}>
                            <h1 style={styles.pgName}>{fullData.nome_pg} {fullData.cognome}</h1>
                            <div style={styles.pgLevel}>Livello {fullData.livello} - {fullData.grado}</div>
                            <div style={styles.infoBar}><span style={{color: THEME.colors.gold}}>PUNTI STAT: {Math.max(0, puntiRimanenti)}</span><span style={{color: THEME.colors.primary}}>EXP: {fullData.exp}</span>{isStaff && <button onClick={handleAdminReset} style={styles.resetBtn}>RESET</button>}</div>
                    ...
                    <div style={styles.contentBody}>
                        ...
                        <div style={styles.rightCol}>
                             ...
                             <div style={styles.planciaGrid}>
                                <div style={styles.statBox}><span style={styles.statBoxLabel}>PESO [PT]</span><span style={styles.statBoxValue}>{derivate.peso_trasportabile.toFixed(1)} kg</span></div>
                                <div style={{...styles.statBox, cursor: 'help', position: 'relative'}} onMouseEnter={() => setHoverLancio(true)} onMouseLeave={() => setHoverLancio(false)}><span style={styles.statBoxLabel}>LANCIO [LAN]</span><span style={styles.statBoxValue}>{derivate.lancio.toFixed(1)} m</span>{hoverLancio && (<div style={styles.tooltipBox}><div style={styles.tooltipRow}><span>Piccole:</span> <span>{(derivate.lancio_base_calc * 0.6).toFixed(1)} m</span></div><div style={styles.tooltipRow}><span>Medie:</span> <span>{(derivate.lancio_base_calc * 0.45).toFixed(1)} m</span></div></div>)}</div>
                                <div style={styles.statBox}><span style={styles.statBoxLabel}>MOVIMENTO [MOV]</span><span style={styles.statBoxValue}>{derivate.movimento.toFixed(1)} m</span></div>
                                <div style={styles.statBox}><span style={styles.statBoxLabel}>SALTO [JUMP]</span><span style={styles.statBoxValue}>{derivate.salto.toFixed(1)} m</span></div>
                             </div>
