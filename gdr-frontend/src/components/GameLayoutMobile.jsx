...
function GameLayoutMobile({ user, onLogout }) {
    ...
    // --- HANDLERS ---
    const handleZoneClick = (child) => {
        if (child.type === 'CHAT') setOpenChat(child); 
        else setMapId(child.id); 
    };
    ...
    const handleCollectSalary = async () => {
        try {
            const res = await api.post('/bank/collect-salary');
    ...
    const handleEnterHouse = async () => {
        try {
            ...
            setOpenChat({ id: res.data.house_chat_id, type: 'CHAT', name: res.data.name || 'Casa Mia' });
    ...
    const handleOpenPrivateChat = (targetUser) => {
        ...
        const mappedUser = {
            ...
            avatar_chat: targetUser.avatar_chat
        };
    ...
    // --- STILI ---
    const containerStyle = { padding: '20px', paddingBottom: '90px', background: '#050508', minHeight: '100vh', color: '#b3b3c0', fontFamily: "'Inter', sans-serif" };
    const headerStyle = { textAlign: 'center', color: '#c9a84a', fontFamily: "'Cinzel', serif", fontSize: '20px', marginBottom: '25px', borderBottom: '1px solid rgba(201, 168, 74, 0.2)', paddingBottom: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', textTransform: 'uppercase', letterSpacing: '1px' };
    const cardStyle = { padding: '15px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', color: '#e6e0ff', borderRadius: '4px', display: 'flex', alignItems: 'center', marginBottom: '10px', transition: 'all 0.2s', cursor: 'pointer' };
    ...
    // --- RENDER CONTENT ---
    const renderContent = () => {
        if (openChat) return <div style={{width:'100%', height:'100%', position:'fixed', top:0, left:0, zIndex:2000}}><ChatWindow chat={openChat} onClose={handleCloseChat} user={user} isMobile={true} /></div>;
        ...
        switch (activeTab) {
            ...
            case 'PRESENTI':
                return (
                    <div style={containerStyle}>
                        <div style={headerStyle}>Presenti ({onlineUsers.length})</div>
                        {onlineUsers.map(u => (
                            <div key={u.id} style={{...cardStyle, justifyContent:'space-between'}}>
